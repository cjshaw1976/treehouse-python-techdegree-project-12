...
def home(request):
    ...
    if request.GET.get('position'):
        ...
        positions = ProjectPosition.objects.filter(title__iexact=show_position).values('project')
        projects = Project.objects.filter(id__in=positions, ).filter(Q(title__icontains=search) | Q(description__icontains=search))
    else:
        # All projects
        projects = Project.objects.filter(Q(title__icontains=search) | Q(description__icontains=search))

    # Get all available positions
    taken = ProjectPositionApplication.objects.filter(status='S').values('position')
    positions = ProjectPosition.objects.filter(project__in=projects).exclude(id__in=taken).values('title').order_by().annotate(Count('title'))
...
def sign_up(request):
    form = forms.SubscriberForm() # UserCreationForm()
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request,
                ("You're now a user! Please click on the confirmation link "
                 "emailed to you to activate your account.")
            )
...
def profile(request, pk=None):
    ...
    positions = ProjectPositionApplication.objects.filter(user=user, status='S')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if current_email != new_email:
                ...
                display_message = 'Please click on the confirmation link emailed to confirm your changed email address.'
...
def list_skills(request):
    ...
    matches = (models.Skill.objects.filter(
        ...
        .exclude(id__in=used) # exclude already used
        .annotate(value=F('title'), data=F('id')) # rename fields
        .values('value', 'data'))
